...
export class ApiMapService {
  ...
  url_api: string =
    'https://api.waqi.info/feed/beijing/?token=dbbd6bd16593d05023748919d281d871c3f79a33';
  ...
  initApi(city: string) {
    return this.http
      .get(`http://localhost:8080/api/ville/${city}`)
      .subscribe((data: any) => {
  ...
  showAllStations(lat1: number, lng1: number, lat2: number, lng2: number) {
    this.http
      .get(
        `http://localhost:8080/api/latlng?lat1=${lat1}&lng1=${lng1}&lat2=${lat2}&lng2=${lng2}`
      )
      .subscribe((data: any): any => {
        ...
        for (let i = 0; i < arrayStations.length; i++) {
          ...
          this.markerMap = L.marker(
            [arrayStations[i].lat, arrayStations[i].lon],
            this.icon
          );
  ...
  markerClick(marker: any) {
    return this.http
      .get(
        `http://localhost:8080/api/markerClick?lat=${marker.latlng.lat}&lng=${marker.latlng.lng}`
      )
      .subscribe((data: any) => {
        this.afficheDonneVille(data);
      });
  ...
  showStationsByName(id: string) {
    this.http
      .get(`http://localhost:8080/api/station/${id}`)
      .subscribe((data: any) => {
        let obj = {
          id: id,
  ...
  showStationsFavoris(stations: string) {
    ...
    this.http
      .get(`http://localhost:8080/api/station/${stations}`)
      .subscribe((data: any) => {
        let obj = {
          latlng: {
